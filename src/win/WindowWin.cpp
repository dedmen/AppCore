    ...
    , is_fullscreen_(fullscreen)
{
    ...
    int windowX = monitor->width() / 2 - windowWidthPx / 2;
    int windowY = monitor->height() / 2 - windowHeightPx / 2;

    // Create window
    RECT rc = { 0, 0, (LONG)windowWidthPx, (LONG)windowHeightPx };
    AdjustWindowRect(&rc, style_, FALSE);
    hwnd_ = ::CreateWindowEx(
        NULL, class_name, _T(""), fullscreen ? (WS_EX_TOPMOST | WS_POPUP) : style_,
        fullscreen ? 0 : windowX, fullscreen ? 0 : windowY,
        fullscreen ? windowWidthPx : (rc.right - rc.left),
        fullscreen ? windowHeightPx : (rc.bottom - rc.top), NULL, NULL, hInstance, NULL);
...
void WindowWin::SetTitle(const char* title)
{
    ...
    ultralight::String16 titleUtf16 = ultralight::String(titleStr.c_str()).utf16();
...
static const char* frameMarker = "WindowWin::Paint";

void WindowWin::MarkBeginFrame()
{
    FrameMarkStart(frameMarker);
    frame_start_time_ = std::chrono::steady_clock::now();
}

void WindowWin::MarkEndFrame()
{
...
void WindowWin::MarkBeginRender()
{
    render_start_time_ = std::chrono::steady_clock::now();
}

void WindowWin::MarkEndRender()
{
    using namespace std::chrono;
    auto now = std::chrono::steady_clock::now();
...
void WindowWin::MarkBeginDraw()
{
    draw_start_time_ = std::chrono::steady_clock::now();
}

void WindowWin::MarkEndDraw()
{
    using namespace std::chrono;
    auto now = std::chrono::steady_clock::now();
...
void WindowWin::UpdateTitleWithStatistics()
{
    ...
    title_stream << (is_accelerated() ? "GPU" : "CPU") << " Renderer | " << fps
