...
void GPUDriverD3D11::CreateTexture(uint32_t texture_id, RefPtr<Bitmap> bitmap) {
  ...
  if (bitmap->IsEmpty()) {
    ...
    hr = context_->device()->CreateTexture2D(&desc, &tex_data,
                                             texture_entry.texture.GetAddressOf());
    bitmap->UnlockPixels();
  }

  if (FAILED(hr)) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::CreateTexture, unable to create texture.", L"Error",
                MB_OK);
  }
  ...
  srv_desc.ViewDimension = texture_entry.is_msaa_render_target ? D3D11_SRV_DIMENSION_TEXTURE2DMS
                                                               : D3D11_SRV_DIMENSION_TEXTURE2D;
  srv_desc.Texture2D.MostDetailedMip = 0;
  srv_desc.Texture2D.MipLevels = 1;

  hr = context_->device()->CreateShaderResourceView(texture_entry.texture.Get(), &srv_desc,
                                                    texture_entry.texture_srv.GetAddressOf());

  if (FAILED(hr)) {
    MessageBoxW(
        nullptr,
        L"GPUDriverD3D11::CreateTexture, unable to create shader resource view for texture.",
        L"Error", MB_OK);
  }
  ...
  if (texture_entry.is_msaa_render_target) {
    ...
    hr = context_->device()->CreateTexture2D(&desc, NULL,
                                             texture_entry.resolve_texture.GetAddressOf());

    if (FAILED(hr)) {
      MessageBoxW(nullptr, L"GPUDriverD3D11::CreateTexture, unable to create MSAA resolve texture.",
                  L"Error", MB_OK);
    }
    ...
    hr = context_->device()->CreateShaderResourceView(
        texture_entry.resolve_texture.Get(), &srv_desc,
        texture_entry.resolve_texture_srv.GetAddressOf());
...
void GPUDriverD3D11::CreateRenderBuffer(uint32_t render_buffer_id, const RenderBuffer& buffer) {
  ...
  if (i != render_targets_.end()) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::CreateRenderBuffer, render buffer id already exists.",
                L"Error", MB_OK);
    return;
  }
  ...
  if (tex_entry == textures_.end()) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::CreateRenderBuffer, texture id doesn't exist.", L"Error",
                MB_OK);
    return;
  }
  ...
  HRESULT hr = context_->device()->CreateRenderTargetView(
      tex.Get(), &renderTargetViewDesc, render_target_entry.render_target_view.GetAddressOf());

  render_target_entry.render_target_texture_id = buffer.texture_id;

  if (FAILED(hr)) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::CreateRenderBuffer, unable to create render target.",
                L"Error", MB_OK);
  }
}
...
void GPUDriverD3D11::BindRenderBuffer(uint32_t render_buffer_id) {
  ...
  if (!target) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::BindRenderBuffer, render buffer id doesn't exist.",
                L"Error", MB_OK);
    return;
  }
...
void GPUDriverD3D11::DrawGeometry(uint32_t geometry_id,
  ...
  if (state.enable_scissor) {
    ...
    D3D11_RECT scissor_rect
        = { (LONG)(state.scissor_rect.left), (LONG)(state.scissor_rect.top),
...
void GPUDriverD3D11::LoadCompiledVertexShader(unsigned char* data,
  ...
  // Create the input layout
  hr = context_->device()->CreateInputLayout(pInputElementDescs, NumElements, data, len,
                                             ppInputLayout);
...
void GPUDriverD3D11::LoadShaders() {
  ...
  const D3D11_INPUT_ELEMENT_DESC layout_2f_4ub_2f[] = {
    ...
    { "COLOR", 0, DXGI_FORMAT_R8G8B8A8_UINT, 0, D3D11_APPEND_ALIGNED_ELEMENT,
      D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
ID3D11RenderTargetView* GPUDriverD3D11::GetRenderTargetView(uint32_t render_buffer_id) {
  ...
  if (i != render_targets_.end()) {
    ...
    if (j == textures_.end()) {
      MessageBoxW(nullptr,
                  L"GPUDriverD3D11::BindRenderBuffer, render target texture id doesn't exist.",
                  L"Error", MB_OK);
      return nullptr;
    }
...
ComPtr<ID3D11SamplerState> GPUDriverD3D11::GetSamplerState() {
  ...
  if (FAILED(hr)) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::GetSamplerState, unable to create sampler state.",
                L"Error", MB_OK);
  }
...
ComPtr<ID3D11Buffer> GPUDriverD3D11::GetConstantBuffer() {
  ...
  if (FAILED(hr)) {
    MessageBoxW(nullptr, L"GPUDriverD3D11::GetConstantBuffer, unable to create constant buffer.",
                L"Error", MB_OK);
  }
...
void GPUDriverD3D11::UpdateConstantBuffer(const GPUState& state) {
  ...
  Matrix model_view_projection
      = ApplyProjection(state.transform, (float)state.viewport_width, (float)state.viewport_height);
...
Matrix GPUDriverD3D11::ApplyProjection(const Matrix4x4& transform,
                                       float screen_width,
                                       float screen_height) {
